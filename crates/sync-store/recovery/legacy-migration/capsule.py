#!/usr/bin/env python3
"""Verify or safely materialize an opaque pondcapsule.legacy.1."""
...
class CapsuleError(ValueError):
    """The capsule violates pondcapsule.legacy.1."""
...
def _validate_manifest(value: Any) -> tuple[dict[str, Any], dict[str, int]]:
    ...
    if manifest["format"] != "pondcapsule.legacy.1":
        raise CapsuleError("manifest format is not pondcapsule.legacy.1")
    source = _keys(
        manifest["source"],
        {
            "pond_id",
    ...
    for index, raw_entry in enumerate(manifest["entries"]):
        ...
        if kind == "directory":
            ...
            _keys(node, {"kind", "recipe"}, f"entries[{index}].node")
