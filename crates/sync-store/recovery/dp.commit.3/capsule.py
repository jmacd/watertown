#!/usr/bin/env python3
"""Verify or safely materialize a pondcapsule.1 without Pond."""
...
ROOT_DOMAIN = b"pondcapsule.root.1\n"
SERIES_DOMAIN = b"pondcapsule.series.1\n"
LEAF_DOMAIN = b"dp.series-leaf.1\n"
ROWS_DOMAIN = b"dp.series-rows.1\n"


class CapsuleError(ValueError):
    """The capsule does not satisfy pondcapsule.1."""
...
def _validate_manifest(value: Any, blake3: Any) -> dict[str, Any]:
    ...
    if manifest["format"] != "pondcapsule.1":
...
def _table_batches(
    ...
    expected = entry["node"]["schema_fingerprint"]
    ...
    for descriptor in entry["node"]["objects"]:
        ...
        if canonical_schema is not None and canonical_schema != encoded:
...
def _partition_batches(
    batches: Iterator[tuple[Any, Any]], counts: list[int]
    ...
    for schema, batch in batches:
        ...
        while offset < batch.num_rows:
            ...
            yield leaf_index, schema, batch.slice(offset, take)
            offset += take
            leaf_rows += take
