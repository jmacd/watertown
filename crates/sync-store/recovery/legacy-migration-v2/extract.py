#!/usr/bin/env python3
"""Extract an opaque pondcapsule.legacy.2 from a local dp.commit.3 backup."""
...
def _extract_graph(
    ...
    for native in native_entries:
        ...
        if entry_type == "dir:physical":
            ...
            if len(native["versions"]) != 1:
                raise FormatError(
                    f"dynamic entry {path} must carry exactly one synthetic metadata version"
                )
            metadata = native["versions"][0]
            if (
                ...
                raise FormatError(
                    f"dynamic entry {path} has unsupported synthetic metadata"
                )
    ...
    manifest = {
        ...
        "source": {
            ...
            "tool_version": "recovery-recipe-legacy-migration.2",
