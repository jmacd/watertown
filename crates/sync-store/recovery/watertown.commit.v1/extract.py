#!/usr/bin/env python3
"""Extract a pondcapsule.2 from a local watertown.commit.v1 Delta backup."""
...
def decode_series(data: bytes) -> dict[str, Any]:
    ...
    if data.startswith(b"dp.series.1\n"):
        ...
        result = {"version": 1, "hashes": [cur.hash() for _ in range(cur.u32())]}
        cur.finish()
        return result
    cur.tag(b"watertown.series.v1\n")
    kind = cur.u8()
    if kind not in (0, 1):
    ...
    result = {
        ...
        "logical_attributes": attributes or None,
...
def _series_root(kind: str, fingerprint: bytes | None, leaves: list[dict[str, Any]], blake3: Any) -> str:
    digest = blake3.blake3(b"pondcapsule.series.2\n")
    ...
    with tempfile.TemporaryDirectory(prefix="watertown-recovery-") as temporary:
        ...
        for native in native_entries:
            ...
            if entry_type == "dir:physical":
                ...
                if series is not None and series["version"] == 1:
                    raise FormatError(
                        f"{paths[native['node_id']]} uses retired dp.series.1 in a watertown.commit.v1 backup"
                    )
                hashes = [child_hash] if series is None else []
                if series is not None:
                    raise FormatError(
                        f"{paths[native['node_id']]} uses watertown.series.v1; "
                        "standalone v2 physical-row/pack recovery is not yet supported"
                    )
        ...
        capsule_root = blake3.blake3(b"pondcapsule.root.2\n" + manifest_bytes).hexdigest()
