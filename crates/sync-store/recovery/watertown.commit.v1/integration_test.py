...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="watertown-recipe-test-") as temporary:
        ...
        try:
            extract(source, destination, "main", None, "fixture")
        except FormatError as error:
            assert "dp.series.1" in str(error), error
            print(f"v2 preflight correctly rejected legacy series object: {error}")
            return 0
        else:
            raise AssertionError("v4 preflight accepted a legacy series object")
        try:
